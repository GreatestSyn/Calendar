...
export const MonthEventsSidebar: React.FC<MonthEventsSidebarProps> = ({
  ...
  return (
    <aside
      ...
      className="bg-white rounded-xl shadow-xs border border-slate-200 flex flex-col h-full overflow-hidden"
    >
      {/* Header */}
      <div className="p-4 border-b border-slate-200 bg-slate-50/70">
        ...
        <div className="flex items-center justify-between text-xs text-slate-600 pt-1">
          ...
          <div className="inline-flex rounded-md bg-slate-200/80 p-0.5" role="tablist">
            <button
              role="tab"
        ...
        <div className="flex flex-wrap gap-1 mt-2.5 pt-2 border-t border-slate-200">
      ...
      {/* Events List */}
      <div className="flex-1 overflow-y-auto p-3 space-y-2 divide-y divide-slate-100" role="list">
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Date & Time */}
                  <div className="flex items-center justify-between text-[11px] text-slate-500 mt-2 gap-1 flex-wrap">
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="font-medium hover:text-indigo-600 hover:underline inline-flex items-center gap-1 text-left"
                      title="View day itinerary"
                    >
                      {(evt.isMultiDay || (evt.endDate && evt.endDate > evt.date)) ? (
                        <CalendarRange className="w-3 h-3 text-indigo-500 shrink-0" />
                      ) : (
                ...
                <div className="space-y-1.5">
                  {catEvents.map((evt) => (
                    <div
                      key={evt.id}
                      onClick={() => onSelectEvent(evt)}
                      className="p-2 bg-white rounded border border-slate-200 hover:border-indigo-300 transition-colors cursor-pointer text-xs"
                    >
                      <div className="font-semibold text-slate-800 hover:text-indigo-600 truncate">
                        {evt.title}
                      </div>
